...
namespace Shipping.SendCloud.Controllers
{
    ...
    public class ShippingSendCloudController : BaseShippingController
    {
        private readonly IWarehouseService _warehouseService;
        private readonly IShippingMethodService _shippingMethodService;
        private readonly IStoreService _storeService;
        private readonly ICountryService _countryService;
        private readonly SendCloudShippingSettings _shippingSendCloudSettings;
        ...
        public ShippingSendCloudController(
            IWarehouseService warehouseService,
            IShippingMethodService shippingMethodService,
            IStoreService storeService,
            ICountryService countryService,
            SendCloudShippingSettings shippingSendCloudSettings,
            ...
            MeasureSettings measureSettings)
        {
            ...
            _shippingSendCloudSettings = shippingSendCloudSettings;
        ...
        {
            var model = new ShippingSendCloudListModel {
                LimitMethodsToCreated = _shippingSendCloudSettings.LimitMethodsToCreated,
                DisplayOrder = _shippingSendCloudSettings.DisplayOrder
            };
        ...
        public async Task<IActionResult> SaveGeneralSettings(ShippingSendCloudListModel model)
        {
            //save settings
            _shippingSendCloudSettings.LimitMethodsToCreated = model.LimitMethodsToCreated;
            _shippingSendCloudSettings.DisplayOrder = model.DisplayOrder;

            await _settingService.SaveSetting(_shippingSendCloudSettings);
        ...
        {
            var records = await _shippingSendCloudService.GetAll(command.Page - 1, command.PageSize);

            var sbwModel = new List<ShippingSendCloudModel>();
            foreach (var x in records)
            {

                var m = new ShippingSendCloudModel
                {
                    Id = x.Id,
                    StoreId = x.StoreId,
                ...
                htmlSb.Append($"{_translationService.GetResource("Plugins.Shipping.SendCloud.Fields.From")}: {m.From}");
                htmlSb.Append("<br />");
                htmlSb.Append($"{_translationService.GetResource("Plugins.Shipping.SendCloud.Fields.To")}: {m.To}");
                htmlSb.Append("<br />");
                htmlSb.Append($"{_translationService.GetResource("Plugins.Shipping.SendCloud.Fields.AdditionalFixedCost")}: {m.AdditionalFixedCost}");
                htmlSb.Append("<br />");
                htmlSb.Append($"{_translationService.GetResource("Plugins.Shipping.SendCloud.Fields.RatePerWeightUnit")}: {m.RatePerWeightUnit}");
                htmlSb.Append("<br />");
                htmlSb.Append($"{_translationService.GetResource("Plugins.Shipping.SendCloud.Fields.LowerWeightLimit")}: {m.LowerWeightLimit}");
                htmlSb.Append("<br />");
                htmlSb.Append($"{_translationService.GetResource("Plugins.Shipping.SendCloud.Fields.PercentageRateOfSubtotal")}: {m.PercentageRateOfSubtotal}");
        ...
        {
            var sbw = await _shippingSendCloudService.GetById(id);
            if (sbw != null)
                await _shippingSendCloudService.DeleteShippingSendCloudRecord(sbw);
        ...
        {
            var model = new ShippingSendCloudModel {
        ...
        [HttpPost]
        [AutoValidateAntiforgeryToken]
        public async Task<IActionResult> AddPopup(ShippingSendCloudModel model)
        {
            var sbw = new ShippingSendCloudRecord
            {
            ...
            await _shippingSendCloudService.InsertShippingSendCloudRecord(sbw);
        ...
        [HttpPost]
        [AutoValidateAntiforgeryToken]
        public async Task<IActionResult> EditPopup(ShippingSendCloudModel model)
        {
            var sbw = await _shippingSendCloudService.GetById(model.Id);
            if (sbw == null)
                //No record found with the specified id
                return RedirectToAction("Configure");
            ...
            await _shippingSendCloudService.UpdateShippingSendCloudRecord(sbw);
